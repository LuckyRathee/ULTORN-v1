...
class CalendarError(UltronError):
...
async def create_calendar_event(
    ...
    if not settings.google_client_id or not settings.google_client_secret:
        raise CalendarError("Google Calendar integration not configured", "auth")
...
async def list_calendar_events(
    ...
    time_max = (end_date or (now + timedelta(days=7))).isoformat() + "Z"
...
async def get_today_events(access_token: Optional[str] = None) -> dict:
    """
    ...
    Returns:
        Dict with events list for today
