...
class BriefingIntent(BaseModel):
    """Trigger daily briefing / morning update."""
    type: Literal[IntentType.BRIEFING]
    city: Optional[str] = None
    confidence: float = Field(..., ge=0.0, le=1.0)


class ChatIntent(BaseModel):
    """General conversation, Q&A, or memory recall response."""
    type: Literal[IntentType.CHAT]
    response: Optional[str] = Field(default=None, description="Direct spoken response or answer to the user")
    confidence: float = Field(..., ge=0.0, le=1.0)
