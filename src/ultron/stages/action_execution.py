...
async def handle_action_execution(state: StateData) -> StateData:
    ...
    try:
        ...
        elif intent.type == IntentType.BRIEFING:
            result = await _execute_briefing(intent)
        elif intent.type == IntentType.CHAT:
            result = await _execute_chat(intent)
...
async def _execute_briefing(intent) -> ActionResult:
    """Execute briefing intent."""
    from ..briefing import generate_daily_briefing, BriefingConfig
    city = getattr(intent, "city", None) or settings.default_city
    cfg = BriefingConfig(city=city)
    briefing = await generate_daily_briefing(cfg)
    return ActionResult(
        success=True,
        data={"briefing_text": briefing.full_text, "audio_url": briefing.audio_url},
        error=None,
        error_type=None,
        latency_ms=0,
    )
...
async def _execute_chat(intent) -> ActionResult:
    """Execute general chat conversation intent."""
    response_text = getattr(intent, "response", None) or "I am here to help! How can I assist you today?"
    return ActionResult(
        success=True,
