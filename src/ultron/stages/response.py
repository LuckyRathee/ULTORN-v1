...
async def handle_response(state: StateData) -> StateData:
    ...
    try:
        # Generate response text if not already set
        ...
        # Post-response hook: save turn into Conversational Memory
        try:
            from .context_injection import get_session_store, get_vector_store
            from ..memory.models import ConversationTurn
            from ..memory.salience import evaluate_salience, turn_to_memory_entry
            ...
            if user_query and response_text:
                ...
                get_session_store().add_turn(turn)

                # Salience scoring for vector memory store
                salience_score = evaluate_salience(turn)
                if salience_score >= 0.5:
                    mem_entry = turn_to_memory_entry(turn, salience_score)
                    get_vector_store().add_memory(mem_entry)
        except Exception as mem_err:
            # Memory logging failure should be non-blocking
            pass
...
def _format_response(state: StateData) -> str:
    ...
    elif state.intent.type == IntentType.BRIEFING:
        return state.action_result.data.get("briefing_text", "Here is your morning briefing.")
    elif state.intent.type == IntentType.CHAT:
        return state.action_result.data.get("response_text", "I'm here to help!")
...
def _format_weather_response(data: dict) -> str:
    """Format weather data into natural, human-spoken language for TTS."""
    if not data:
    ...
    parts = [f"The weather in {location} is currently {condition}"]
    
    if temp is not None:
        parts.append(f"with a temperature of {int(round(temp))} degrees Celsius")
    
    if humidity is not None:
        parts.append(f"and {humidity} percent humidity")
    
    if wind is not None:
        parts.append(f"with winds around {int(round(wind))} kilometers per hour")
